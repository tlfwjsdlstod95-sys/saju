...
- 구조 패턴(이 사람만의 결, 반드시 반영): ${patStr}` +
    ...
    (s12Str ? `\n- 12신살(띠 기준, 이 사람에게 실제로 걸린 것): ${s12Str} — 살 이름을 그대로 나열하지 말고, 그 기운이 일상에서 어떻게 드러나는지로 풀 것` : '') +
    (gy.yongsin.method === '병약'
      ? `\n- ※ 이 사람은 병약용신(病藥) 구조: 한 기운이 극단으로 몰린 것이 '병'이고 그걸 덜어내는 게 '약'. 강약 보완(억부)보다 '편중을 먼저 푼다'는 관점으로 조언할 것`
      : gy.yongsin.method === '통관'
        ? `\n- ※ 이 사람은 통관용신(通關) 구조: 두 기운이 팽팽히 맞서 흐름이 막혀 있음. '어느 쪽을 이기느냐'가 아니라 '사이를 잇는 다리를 놓는다'는 관점으로 조언할 것(중재·연결·전환의 자리에서 풀린다)`
        : '') +
    // 기준이 갈리는 명식은 AI 가 단정하지 않도록 근거를 같이 넘긴다.
    // (사용자가 다른 앱과 비교했을 때 '틀렸다'가 아니라 '기준이 다르다'로 읽히게 하는 장치)
    (gy.yongsin.conflict
      ? `\n- ※ 이 명식은 기준(법)에 따라 용신이 갈린다: ${gy.yongsin.bases.filter((b) => b.value).map((b) => `${b.method} ${b.value}${b.adopted ? '(채택)' : ''}`).join(' / ')}. `
        + `우리는 ${gy.yongsin.method} 기준을 채택했다. 조언은 채택한 용신(${gy.yongsin.primary}) 기준으로 쓰되, `
        + `단정적으로 "당신의 용신은 하나다"라고 못 박지 말 것. 필요하면 "보는 기준에 따라 달리 볼 수도 있다"는 한 마디만 곁들일 것(장황한 설명 금지)`
      : '');
}
...
- 각 섹션 분량 가이드:
  ...
  thisyear 4~5문장(올해 상반기·하반기 구분, 뭘 하고 뭘 조심) / love 7~9문장(가장 공들여 길게: 연애 스타일·매력 포인트·반복하는 실수·'끌리는 사람'과 '잘 맞는 사람'의 차이(용신 기준 이상형)·갈등 포인트·올해 인연운과 배우자궁 작용까지 구체적으로) /
